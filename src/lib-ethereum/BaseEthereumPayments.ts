    ...
    >
{
  ...
  resolveCustomFeeOption(feeOption: FeeOptionCustom, amountOfGas: number): EthereumResolvedFeeOption {
    ...
    if (feeRateType === FeeRateType.BasePerWeight) {
      ...
      const feeRateBase =
        feeRateType === FeeRateType.Main ? this.toBaseDenominationBigNumberEth(feeRate) : new BigNumber(feeRate)
  ...
  private async createTransactionObject(
    ...
    if (serviceFlag) {
      if (options.data) {
        ...
        txConfig.data = TOKEN_PROXY_DATA.replace(
          /<address to proxy>/g,
          options.proxyAddress.replace('0x', '').toLowerCase(),
        )
      } else {
