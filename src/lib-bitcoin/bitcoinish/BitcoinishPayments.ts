...
export abstract class BitcoinishPayments<Config extends BaseConfig>
  extends BitcoinishPaymentsUtils
  implements
    BasePayments<
      Config,
      BitcoinishUnsignedTransaction,
    ...
    >
{
  ...
  private reallocateLooseChange(tbc: BitcoinishTxBuildContext, changeOutputCount: number, looseChange: number): number {
    ...
    if (extraSatPerChangeOutput > 0) {
      this.logger.log(
        `${this.coinSymbol} buildPaymentTx - allocating ${
          extraSatPerChangeOutput * changeOutputCount
        } sat loose change ` + `across ${changeOutputCount} change outputs (${extraSatPerChangeOutput} sat each)`,
      )
      for (let i = 0; i < changeOutputCount; i++) {
