...
> {
  ...
  private async createConnection<T extends U | P | B>(config: C, instantiator: (c: C) => T): Promise<T> {
    const connected = instantiator(config)
    await connected.init()
    return connected
  }

  private async reuseConnection<T extends U | P | B>(config: C, instantiator: (c: C) => T): Promise<T> {
