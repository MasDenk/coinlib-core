...
import { assertType, isNil, Numeric, isString, toBigNumber } from '../ts-common'
...
export abstract class BaseStellarPayments<Config extends BaseStellarPaymentsConfig>
  extends StellarPaymentsUtils
  implements
    BasePayments<
      Config,
      StellarUnsignedTransaction,
    ...
    >
{
  ...
  private async doCreateTransaction(
    ...
    const { requiresActivation: fromAddressRequiresActivation, confirmedBalance: fromAddressBalance } =
  ...
  async broadcastTransaction(signedTx: StellarSignedTransaction): Promise<StellarBroadcastResult> {
    ...
    try {
      result = await limiter.schedule(() => this.getApi().submitTransaction(preparedTx))
    } catch (e) {
