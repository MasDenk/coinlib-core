...
import { isNil, assertType, Numeric, isMatchingError } from '../ts-common'
...
export class StellarPaymentsUtils extends StellarConnected implements PaymentsUtils {
  ...
  async getFeeRateRecommendation(level: AutoFeeLevels): Promise<FeeRate> {
    const feeStats = await limiter.schedule(() => this.getApi().feeStats())
  ...
  async getCurrentBlockNumber() {
    return limiter.schedule(async () => (await this.getBlock()).height)
  }
  ...
  async loadAccount(address: string) {
    let accountInfo
    try {
      accountInfo = await limiter.schedule(() => this.getApi().loadAccount(address))
    } catch (e) {
  ...
  async getLatestBlock(): Promise<Stellar.ServerApi.LedgerRecord> {
    const page = await limiter.schedule(() => this.getApi().ledgers().order('desc').limit(1).call())
    if (!page.records) {
  ...
  async getTransactionInfo(txId: string): Promise<StellarTransactionInfo> {
    ...
    try {
      tx = await limiter.schedule(() => this.getApi().transactions().transaction(txId).call())
    } catch (e) {
    ...
    const ledger = await limiter.schedule(() => tx.ledger())
