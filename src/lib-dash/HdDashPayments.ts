...
export class HdDashPayments extends SinglesigDashPayments<HdDashPaymentsConfig> {
  readonly derivationPath: string
  readonly xpub: string
  readonly xprv: string | null
  readonly hdNode: HDNode

  constructor(private config: HdDashPaymentsConfig) {
    super(config)
    assertType(HdDashPaymentsConfig, config)
