...
exports.MultisigDogePayments = void 0;
...
class MultisigDogePayments extends BaseDogePayments_1.BaseDogePayments {
    constructor(config) {
        super(config);
        this.config = config;
        ...
        this.signers = config.signers.map((signerConfig, i) => {
            ...
            if (signerConfig.network !== this.networkType) {
                throw new Error(`MultisigDogePayments is on network ${this.networkType} but signer config ${i} is on ${signerConfig.network}`);
            }
            const payments = types_1.HdDogePaymentsConfig.is(signerConfig)
                ? new HdDogePayments_1.HdDogePayments(signerConfig)
                : new KeyPairDogePayments_1.KeyPairDogePayments(signerConfig);
...
exports.MultisigDogePayments = MultisigDogePayments;
exports.default = MultisigDogePayments;
//# sourceMappingURL=MultisigDogePayments.js.map