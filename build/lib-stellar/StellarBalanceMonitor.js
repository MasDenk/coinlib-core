...
exports.StellarBalanceMonitor = void 0;
...
class StellarBalanceMonitor extends StellarConnected_1.StellarConnected {
    constructor() {
        super(...arguments);
    ...
    onBalanceActivity(callbackFn) {
        this.txEmitter.on('tx', ({ address, tx }) => {
            this.txToBalanceActivity(address, tx)
                ...
                .catch((e) => this.logger.error(`Error in Stellar ${this.networkType} onBalanceActivity handling of new activity`, e));
        });
    }
    async retrieveBalanceActivities(address, callbackFn, options = {}) {
        (0, helpers_1.assertValidAddress)(address);
        const { from: fromOption, to: toOption } = options;
        const from = new lib_common_1.BigNumber((0, ts_common_1.isUndefined)(fromOption) ? 0 : ts_common_1.Numeric.is(fromOption) ? fromOption : fromOption.confirmationNumber);
        const to = new lib_common_1.BigNumber((0, ts_common_1.isUndefined)(toOption) ? 'Infinity' : ts_common_1.Numeric.is(toOption) ? toOption.toString() : toOption.confirmationNumber);
        const limit = 10;
        let lastTx;
        let transactionPage;
        let transactions;
        while ((0, ts_common_1.isUndefined)(transactionPage) ||
            ...
            try {
                transactionPage = await lib_common_1.limiter.schedule(() => transactionPage
    ...
    async txToBalanceActivity(address, tx) {
        ...
        const primarySequence = (0, utils_1.padLeft)(String(tx.ledger_attr), 12, '0');
        const secondarySequence = (0, utils_1.padLeft)(String(new Date(tx.created_at).getTime()), 18, '0');
        ...
        return [
            {
                ...
                assetSymbol: 'XLM',
                address: address,
                extraId: !(0, ts_common_1.isUndefined)(extraId) ? extraId : null,
