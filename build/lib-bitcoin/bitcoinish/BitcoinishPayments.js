...
exports.BitcoinishPayments = void 0;
const lib_common_1 = require("../../lib-common");
const ts_common_1 = require("../../ts-common");
const t = __importStar(require("io-ts"));
const utils_1 = require("./utils");
const BitcoinishPaymentsUtils_1 = require("./BitcoinishPaymentsUtils");
class BitcoinishPayments extends BitcoinishPaymentsUtils_1.BitcoinishPaymentsUtils {
    constructor(config) {
        super(config);
        this.minTxFee = config.minTxFee;
        this.dustThreshold = config.dustThreshold;
        this.defaultFeeLevel = config.defaultFeeLevel;
        this.targetUtxoPoolSize = (0, ts_common_1.isUndefined)(config.targetUtxoPoolSize) ? 1 : config.targetUtxoPoolSize;
        const minChange = (0, ts_common_1.toBigNumber)((0, ts_common_1.isUndefined)(config.minChange) ? 0 : config.minChange);
        if (minChange.lt(0)) {
    ...
    async resolvePayport(payport) {
        ...
        else if (lib_common_1.DerivablePayport.is(payport)) {
    ...
    async resolveFeeOption(feeOption) {
        ...
        if ((0, ts_common_1.isType)(lib_common_1.FeeOptionCustom, feeOption)) {
            targetLevel = lib_common_1.FeeLevel.Custom;
            target = feeOption;
        }
        else {
        ...
        if (target.feeRateType === lib_common_1.FeeRateType.Base) {
            feeBase = target.feeRate;
            feeMain = this.toMainDenominationString(feeBase);
        }
        else if (target.feeRateType === lib_common_1.FeeRateType.Main) {
    ...
    feeRateToSatoshis({ feeRate, feeRateType }, inputCount, changeOutputCount, externalOutputAddresses) {
        ...
        else if (feeRateType === lib_common_1.FeeRateType.Main) {
            return this.toBaseDenominationNumber(feeRate);
        }
        return Number.parseFloat(feeRate);
    }
    /** Estimate the tx fee in satoshis */
    estimateTxFee(targetRate, inputCount, changeOutputCount, externalOutputAddresses) {
    ...
    adjustTxFee(tbc, newFeeSat, description) {
        ...
        if (newFeeSat > Math.ceil(tbc.desiredOutputTotal * (tbc.maxFeePercent / 100))) {
            throw new lib_common_1.PaymentsError(lib_common_1.PaymentsErrorCode.TxFeeTooHigh, `${description} (${newFeeSat} sat) exceeds maximum fee percent (${tbc.maxFeePercent}%) ` +
        ...
        this.adjustOutputAmounts(tbc, tbc.externalOutputTotal - feeSatAdjustment, description, (m) => new lib_common_1.PaymentsError(lib_common_1.PaymentsErrorCode.TxFeeTooHigh, m));
        this.applyFeeAdjustment(tbc, feeSatAdjustment, description);
    }
    ...
    selectInputUtxos(tbc) {
        ...
        if (tbc.externalOutputTotal + tbc.feeSat > tbc.inputTotal) {
            throw new lib_common_1.PaymentsError(lib_common_1.PaymentsErrorCode.TxInsufficientBalance, `${this.coinSymbol} buildPaymentTx - You do not have enough UTXOs (${tbc.inputTotal} sat) ` +
    ...
    estimateIdealUtxoSelectionFee(tbc, inputCount) {
    ...
    selectWithForcedUtxos(tbc) {
        ...
        const feeSat = this.estimateTxFee(tbc.desiredFeeRate, tbc.inputUtxos.length, targetChangeOutputCount, tbc.externalOutputAddresses);
    ...
    selectFromAvailableUtxos(tbc) {
        ...
        for (const utxo of (0, utils_1.shuffleUtxos)(tbc.selectableUtxos)) {
            tbc.inputUtxos.push(utxo);
            tbc.inputTotal += utxo.satoshis;
            const targetChangeOutputCount = this.determineTargetChangeOutputCount(tbc.nonDustUtxoCount, tbc.inputUtxos.length);
            feeSat = this.estimateTxFee(tbc.desiredFeeRate, tbc.inputUtxos.length, targetChangeOutputCount, tbc.externalOutputAddresses);
    ...
    validateBuildContext(tbc) {
        const inputSum = (0, utils_1.sumField)(tbc.inputUtxos, 'satoshis');
        ...
        const externalOutputSum = (0, utils_1.sumField)(tbc.externalOutputs, 'satoshis');
        ...
        const changeOutputSum = (0, utils_1.sumField)(tbc.changeOutputs, 'satoshis');
    ...
    async buildPaymentTx(params) {
        ...
        const tbc = {
            ...
            selectableUtxos: nonDustUtxos
                .filter((utxo) => params.useUnconfirmedUtxos || (0, utils_1.isConfirmedUtxo)(utxo))
        ...
        const unfilteredUtxoTotal = (0, utils_1.sumUtxoValue)(params.unusedUtxos, params.useUnconfirmedUtxos);
        ...
        const estimatedWeight = this.estimateTxSize(tbc.inputUtxos.length, tbc.changeOutputs.length, tbc.externalOutputAddresses);
        ...
        return {
            ...
            changeAddress: tbc.changeOutputs.length === 1 ? tbc.changeOutputs[0].address : null,
    ...
    allocateChangeUsingWeights(tbc, changeOutputWeights) {
        let totalChangeWeight = (0, utils_1.sumField)(changeOutputWeights, 'weight').toNumber();
        ...
        for (let i = 0; i < changeOutputWeights.length; i++) {
            ...
            if (changeSat <= this.dustThreshold || changeSat < this.minChangeSat) {
                this.logger.debug(`${this.coinSymbol} buildPaymentTx - desired change output ${i} with weight ` +
    ...
    readjustTxFeeAfterDroppingChangeOutputs(tbc, changeOutputCount, totalChangeAllocated, looseChange) {
        const recalculatedFee = this.estimateTxFee(tbc.desiredFeeRate, tbc.inputUtxos.length, changeOutputCount || 1, // minimum one change output for loose change
        ...
        this.logger.log(`${this.coinSymbol} buildPaymentTx - Adjusted looseChange from ` +
            `${looseChangeBefore} sat to ${looseChange} sat after ` +
            'applying dropped change outputs recalculated fee');
        return looseChange;
    }
    reallocateLooseChange(tbc, changeOutputCount, looseChange) {
        const extraSatPerChangeOutput = Math.floor(looseChange / changeOutputCount);
        if (extraSatPerChangeOutput > 0) {
            this.logger.log(`${this.coinSymbol} buildPaymentTx - allocating ${extraSatPerChangeOutput * changeOutputCount} sat loose change ` + `across ${changeOutputCount} change outputs (${extraSatPerChangeOutput} sat each)`);
            for (let i = 0; i < changeOutputCount; i++) {
    ...
    async callSerializePaymentTx(paymentTx, options) {
        var _a;
        const inputHashes = paymentTx.inputs.map(({ txid }) => txid);
        const txHashToDataHex = await ((_a = options === null || options === void 0 ? void 0 : options.lookupTxDataByHashes) === null || _a === void 0 ? void 0 : _a.call(options, inputHashes));
        return this.serializePaymentTx({
            ...paymentTx,
            inputs: paymentTx.inputs.map(({ txid, txHex, ...rest }) => ({
                ...rest,
                txid,
                txHex: txHex !== null && txHex !== void 0 ? txHex : txHashToDataHex === null || txHashToDataHex === void 0 ? void 0 : txHashToDataHex[txid],
            })),
        }, options.fromIndex);
    }
    ...
    async createMultiOutputTransaction(from, to, options = {}) {
        var _a, _b, _c, _d;
        (0, ts_common_1.assertType)(t.array(lib_common_1.PayportOutput), to);
        ...
        const maxFeePercent = new lib_common_1.BigNumber((_a = options.maxFeePercent) !== null && _a !== void 0 ? _a : lib_common_1.DEFAULT_MAX_FEE_PERCENT).toNumber();
        ...
        const paymentTx = await this.buildPaymentTx({
            ...
            useAllUtxos: (_b = options.useAllUtxos) !== null && _b !== void 0 ? _b : false,
            useUnconfirmedUtxos: (_c = options.useUnconfirmedUtxos) !== null && _c !== void 0 ? _c : false,
            recipientPaysFee: (_d = options.recipientPaysFee) !== null && _d !== void 0 ? _d : false,
            maxFeePercent,
        });
        ...
        paymentTx.rawHash = (0, utils_1.sha256FromHex)(unsignedTxHex);
        ...
        if (paymentTx.externalOutputs.length === 1) {
            ...
            resultToIndex = (0, ts_common_1.isNumber)(to[0].payport) ? to[0].payport : null;
        }
        return {
    ...
    async createServiceTransaction(from, options = {}) {
        return null;
    }
    async createSweepTransaction(from, to, options = {}) {
        this.logger.debug('createSweepTransaction', from, to, options);
        const availableUtxos = (0, ts_common_1.isUndefined)(options.availableUtxos) ? await this.getUtxos(from) : options.availableUtxos;
        const forcedUtxos = (0, ts_common_1.isUndefined)(options.forcedUtxos) ? [] : options.forcedUtxos;
        ...
        const useUnconfirmedUtxos = (0, ts_common_1.isUndefined)(options.useUnconfirmedUtxos) ? true : options.useUnconfirmedUtxos;
        const outputAmount = (0, utils_1.sumUtxoValue)(availableUtxos, useUnconfirmedUtxos).plus((0, utils_1.sumUtxoValue)(forcedUtxos, true));
    ...
    async createMultiInputTransaction(from, to, options = {}) {
        ...
        const maxFeePercent = new lib_common_1.BigNumber((_a = options.maxFeePercent) !== null && _a !== void 0 ? _a : lib_common_1.DEFAULT_MAX_FEE_PERCENT).toNumber();
        const { targetFeeLevel, targetFeeRate, targetFeeRateType } = await this.resolveFeeOption(options);
        this.logger.debug(`createMultiInputTransaction resolvedFeeOption ${targetFeeLevel} ` + `${targetFeeRate} ${targetFeeRateType}`);
        const paymentTx = await this.buildPaymentTx({
            unusedUtxos,
            enforcedUtxos: options.forcedUtxos || [],
            desiredOutputs,
    ...
    async broadcastTransaction(tx) {
        let txId;
        try {
            txId = await lib_common_1.limiter.schedule(() => this.getApi().sendTx(tx.data.hex));
            if (tx.id !== txId) {
    ...
    prepareUtxos(utxos) {
        return utxos.map((utxo) => {
            return {
                ...utxo,
                satoshis: (0, ts_common_1.isUndefined)(utxo.satoshis)
                    ? this.toBaseDenominationNumber(utxo.value)
                    : (0, ts_common_1.toBigNumber)(utxo.satoshis).toNumber(),
            };
        });
    }
    validatePsbtOutput(output, psbtOutput, i) {
    ...
    validatePsbtInput(input, psbtInput, i) {
        ...
        if (input.vout !== psbtInput.index) {
            throw new Error(`Invalid tx: psbt input ${i} index (${psbtInput.index}) doesn't match expected vout (${input.vout})`);
        }
    }
    /**
    ...
    validatePsbt(tx, psbt) {
        ...
        if (inputUtxos.length !== data.inputs.length) {
            throw new Error(`Invalid tx: inputUtxos length (${psbtInputs.length}) doesn't match data.inputs length (${data.inputs.length})`);
        }
        if (psbtInputs.length !== data.inputs.length) {
            throw new Error(`Invalid tx: psbt inputs length (${psbtInputs.length}) doesn't match data.inputs length (${data.inputs.length})`);
        }
        let inputTotal = new lib_common_1.BigNumber(0);
        ...
        if (externalOutputs.length !== data.externalOutputs.length) {
            throw new Error(`Invalid tx: externalOutputs length (${externalOutputs.length}) doesn't match data.externalOutputs length (${data.externalOutputs.length})`);
        }
        const expectedOutputCount = data.externalOutputs.length + data.changeOutputs.length;
        if (psbtOutputs.length !== expectedOutputCount) {
            throw new Error(`Invalid tx: psbt outputs length (${psbtOutputs.length}) doesn't match external + change output length (${expectedOutputCount})`);
        }
        ...
        let externalOutputTotal = new lib_common_1.BigNumber(0); // main denom
        let changeOutputTotal = new lib_common_1.BigNumber(0); // main denom
